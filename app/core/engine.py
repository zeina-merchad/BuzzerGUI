...
class GameEngine(QObject):
    ...
    def __init__(self, cfg: GameConfig, questions: List[Question]):
        ...
        # In real runs this should come from heartbeat / MQTT.
        # For local simulation, you can keep all four active.
    ...
    def set_active_players(self, player_ids) -> None:
        ...
        self._active_player_ids.update(added)
    ...
    def start_or_resume_question_timer(self) -> None:
        ...
        if not self.get_players_remaining():
            ...
            self.error_occurred.emit("No live buzzers connected for this question")
            return
    ...
    def _on_question_timer_ended(self) -> None:
        ...
        if self.cfg.enable_cascading_attempts and self.current_attempt_number < max_attempts:
            self.current_attempt_number += 1
            self.attempt_changed.emit(self.current_attempt_number)
    ...
    def _restore_state_snapshot(self, snapshot: dict) -> None:
        ...
        self.attempt_records = [
            AttemptRecord(**vars(r)) if isinstance(r, AttemptRecord) else AttemptRecord(**r)
    ...
    def apply_answer(self, is_correct: bool, answering_player_id: Optional[int] = None) -> None:
        ...
        if (
            ...
            print(f"[ENGINE] → Next attempt available ({self.current_attempt_number}/{max_attempts})")
