...
class Question:
    ...
    # Cascading points for multiple attempts
...
class GameConfig:
    ...
    # FIX #7: tuple is truly immutable, unlike list, consistent with frozen=True
    ...
    # Global scoring defaults (used when creating new questions)
    ...
    def validate(self) -> tuple[bool, str]:
        ...
        FIX #5: question_files is allowed to be empty here because the
        'start with no pack' flow (admin loads Excel later) deliberately
        creates a GameConfig with an empty question_files list.
        The loader validates non-emptiness at load time instead.
        """
        if self.rounds <= 0:
...
class GameStats:
    ...
    def record_answer(self, is_correct: bool, player_id: int, buzz_time_ms: int, attempt_number: int = 1):
        ...
        # FIX #12: clean None-safe comparison
