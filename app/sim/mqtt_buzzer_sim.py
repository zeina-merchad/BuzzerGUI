...
class MQTTBuzzerBackend:
    ...
    def _handle_buzz(self, player_id: int, data: dict):
        ...
        # Track latency
        if player_id not in self.player_latency:
            self.player_latency[player_id] = []
        self.player_latency[player_id].append(buzz_event.latency_ms)
        
        print(f"\n🔔 BUZZ from Player {player_id} (latency: {buzz_event.latency_ms}ms)")
    ...
    def _handle_answer(self, player_id: int, data: dict):
        ...
        if self.state != BuzzerState.LOCKED:
            print(f"  ⚠ Answer rejected: State is {self.state.value}, not LOCKED")
            return
        
        if player_id != self.locked_player:
            print(f"  ⚠ Answer rejected: Player {player_id} not locked in (locked: {self.locked_player})")
            return
        
        if player_id in self.answered_players:
            print(f"  ⚠ Answer rejected: Player {player_id} already answered")
            return
        ...
        print(f"  ✓ Answer accepted from Player {player_id}")
    ...
    def simulate_buzz(self, player_id: int):
        """Simulate a buzzer press (for testing)"""
        topic = f"fbz/buzzer/{player_id}/buzz"
        payload = json.dumps({
            "id": player_id,
            "t_ms": int(time.time() * 1000)
        })
        self.client.publish(topic, payload)
        print(f"🧪 Simulated buzz from Player {player_id}")
    
    def simulate_answer(self, player_id: int, answer: str):
        """Simulate an answer (for testing)"""
        topic = f"fbz/buzzer/{player_id}/answer"
        payload = json.dumps({
            "id": player_id,
            "answer": answer.upper(),
            "t_ms": int(time.time() * 1000)
        ...
        self.client.publish(topic, payload)
        print(f"🧪 Simulated answer from Player {player_id}: {answer}")
...
if __name__ == "__main__":
    ...
    print("  test <N>  - Simulate buzz from player N")
    print("  quit      - Exit")
    print("="*60 + "\n")
    ...
    try:
        while True:
            ...
            if cmd[0] == "start":
                backend.start_question("test_question", max_attempts=3)
                ...
                backend.simulate_buzz(int(cmd[1]))
