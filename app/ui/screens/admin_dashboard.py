...
class QuestionListItem(QFrame):
    ...
    def __init__(self, question: Question, index: int, enabled: bool = True):
        ...
        for t, c in [
            ...
            lbl = QLabel(t)
            lbl.setStyleSheet(f"font-size: 11px; font-weight: 700; color: {c};")
            meta.addWidget(lbl)
...
class AdminDashboard(QWidget):
    ...
    def _create_settings_tab(self):
        ...
        lbl = QLabel("⚙️ GAME CONFIGURATION")
        lbl.setStyleSheet("font-size: 18px; font-weight: 900; color: #39FF14;")
        lay.addWidget(lbl)
    ...
    def _qlist(self):
        ...
        filter_lbl.setStyleSheet("font-size: 12px; color: rgba(255,255,255,0.7);")
        ...
        h.addStretch()
        for widget in (filter_lbl, self.filter_combo, ba, bn, bw):
            h.addWidget(widget)
        lay.addLayout(h)
    ...
    def _editor(self):
        ...
        self.et = QTextEdit()
        self.et.setPlaceholderText("Enter question text...")
    ...
    # ── CONFIG DICT (from Excel Config sheet) ────────────────────────────────
    ...
    def _ref(self):
        ...
        if visible_ids != cached_ids:
            while self.qlay.count():
                item = self.qlay.takeAt(0)
                if item.widget():
                    item.widget().setParent(None)
            self._item_cache.clear()
            for i, q, enabled in filtered:
    ...
    def _sv_q(self):
        ...
        if self._game_active:
            ...
            return
        self._sync_to_engine()
        self.status.setText("✅ Question saved")
    ...
    # FIX #1: _sp() was truncated in the original file — method is now complete.
